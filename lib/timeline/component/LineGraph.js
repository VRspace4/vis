...
var Boxes = require('./graph2d_types/box');
...
LineGraph.prototype._updateAllGroupData = function (ids, groupIds) {
  if (this.itemsData != null) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      extended.startDate = util.convert(item.startDate, 'Date');
      extended.endDate = util.convert(item.endDate, 'Date');
      extended.startVal = Number(item.startVal);
      extended.endVal = Number(item.endVal);
        ...
        if (groupsData[groupIds[i]].length > 0) {
          switch (group.options.style) {
            ...
            case "points":
              ...
              break;
            case "box":
              Boxes.draw(groupIds, groupsData, this.framework);
            case "bar":
...
LineGraph.prototype._convertXcoordinates = function (datapoints) {
  ...
  for (var i = 0; i < datapoints.length; i++) {
    ...
    datapoints[i].screen_startDate = toScreen(datapoints[i].startDate) + this.props.width;
    datapoints[i].screen_endDate = toScreen(datapoints[i].endDate) + this.props.width;
    datapoints[i].screen_y = datapoints[i].y; //starting point for range calculations
	datapoints[i].screen_startVal = datapoints[i].startVal; //starting point for range calculations
	datapoints[i].screen_endVal = datapoints[i].endVal; //starting point for range calculations
  }
};
...
LineGraph.prototype._convertYcoordinates = function (datapoints, group) {
  ...
  for (var i = 0; i < datapoints.length; i++) {
    ...
    if (this.options.style === 'box') {
      datapoints[i].screen_startVal = Math.round(axis.convertValue(datapoints[i].startVal));
	  datapoints[i].screen_endVal = Math.round(axis.convertValue(datapoints[i].endVal));
    }
  }
